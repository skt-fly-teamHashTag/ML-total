...
def get_keyshot_summ(pred: np.ndarray,
    ...
    for seg_idx, (first, last) in enumerate(cps):
        ...
        seg_scores[seg_idx] = int(1000 * scores.mean()) #세그먼트 내의 프레임들의 점수 평균 

    # Apply knapsack algorithm to find the best shots
    limits = int(n_frames * proportion) #proportion: float = 0.15
    packed = knapsack(seg_scores, nfps, limits) 

    # Get key-shot based summary
    ## knapsack 을 통해 limits 이상의 score을 가지는 segment(group of frames)에만 1, else 0 
    summary = np.zeros(n_frames, dtype=np.bool)
    thumb_nail = np.zeros(n_frames, dtype=np.bool)
    thumb_nail_scores = {}
    for seg_idx in packed:
        ...
        ## sum_nail frame idx 
        max_frame_idx = frame_scores[first:last+1].argmax() + first
        thumb_nail[max_frame_idx] = True 
        thumb_nail_scores[max_frame_idx] = (seg_scores[seg_idx], frame_scores[max_frame_idx])#(seg_score, frame_score)

    return summary, thumb_nail, thumb_nail_scores 
